...
function no_obs_check_search(planner::SparsePFTPlanner, b_idx::Int, d::Int)
    tree = planner.tree
    pomdp = planner.pomdp
    sol = planner.sol
    ...
    if length(tree.ba_children[ba_idx]) ≤ sol.k_o
        bp, o, r = GenBelief(planner, pomdp, tree.b[b_idx], a)
        n_b = insert_belief!(tree, bp, ba_idx, o, r, planner)
        total = r + discount(pomdp)*no_obs_check_search(planner, n_b, d-1)
    else
        bp_idx = rand(sol.rng, tree.ba_children[ba_idx])
        r = tree.b_rewards[bp_idx]
        total = r + discount(pomdp)*no_obs_check_search(planner, bp_idx, d-1)
...
function obs_check_search(planner::SparsePFTPlanner, b_idx::Int, d::Int)
    tree = planner.tree
    pomdp = planner.pomdp
    sol = planner.sol
