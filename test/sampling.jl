@testset "Non-terminal Sampling" begin
    ...
    b = SparseParticleTrees.PFTBelief([0,2,0,4], Float64[0.20,0.25,0.25,0.25], pomdp)
    rng = Random.GLOBAL_RNG
    N = 50_000
    samples = [SparseParticleTrees.non_terminal_sample(rng, pomdp, b) for _ in 1:N]

    for (s,w) in SparseParticleTrees.weighted_particles(b)
        c = count(==(s), samples)
        ratio = c/N
