...
@testset "SubHunt Terminal" begin
    ...
    w = fill(1/10,10)
    w[2] = 0.0
    test_b = ParticleFilterTrees.PFTBelief{SubState}(p, w, 0.0)

    push!(subhunt_planner.tree.b, test_b)
    ParticleFilterTrees.freenext!(subhunt_planner.tree.b_children)
    push!(subhunt_planner.tree.Nh, 0)
    push!(subhunt_planner.tree.b_rewards, 0.0)

    @test try
        ParticleFilterTrees.no_obs_check_search(subhunt_planner, 1, 10)
        true
    catch e
        println(e)
        false
